      ...
      if (stmt.where_condition->logic_operator == TokenType::AND) {
        matches = matches &&
                  compareValues(
                      ...
                      convertTokenToValue(stmt.where_condition->value_b), row);
  ...
  for (auto &row : rows) {
    ...
    if (matches) {
      ...
      for (const auto &set_condition : stmt.set_conditions) {
        ...
        if (it == column_index.end()) {
          throw TableError("Column not found: " + set_condition.column_name_a);
        }
        size_t index = it->second;
...
void Table::deleteRows(const DeleteStatement &stmt) {
  ...
  while (it != rows.end()) {
    ...
    should_delete =
        compareValues(stmt.where_condition->column_name_a,
                      stmt.where_condition->condition_type_a,
...
void Table::innerJoin(const InnerJoinStatement &stmt, Table &table_b) {
